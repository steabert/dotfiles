...
function! MyGdiff(...)
    ...
    cexpr system('git diff ' . src . ' HEAD -U0 | ~/.config/nvim/sbin/diff2qf')
    copen
endfunction
...
nnoremap <silent>glj	<cmd>lua vim.diagnostic.goto_next{ wrap = true }<CR>
nnoremap <silent>glk	<cmd>lua vim.diagnostic.goto_prev{ wrap = true }<CR>
nnoremap <silent>gll	<cmd>lua vim.diagnostic.setloclist()<CR>
nnoremap <silent>glq	<cmd>lua vim.diagnostic.setqflist()<CR>
